...
rule parse_clusterprofiler_enrichment:
    input:
        enrichment="{project}/enrichment/clusterprofiler/STAR_FC_{dataset}_enrichment.rds",
    output:
        discovery_go="{project}/enrichment/clusterprofiler/{dataset}_go_enrichment.tsv",
        discovery_kegg="{project}/enrichment/clusterprofiler/{dataset}_kegg_enrichment.tsv",
        discovery_others="{project}/enrichment/clusterprofiler/{dataset}_others_enrichment.tsv",
    log:
        "logs/{project}/parse_enrichment_{dataset}_clusterprofiler.log",
    container:
        (
            "docker://btrspg/clusterprofiler:4.14.0"
            if config["container"].get("clusterprofiler", None) is None
