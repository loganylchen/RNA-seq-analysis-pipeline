...
_tmp_df = (
    pd.read_csv(files[0], sep="\t", comment="#", index_col=0).iloc[:, 5].to_frame()
)
for f in files[1:]:
    _tmp_df = _tmp_df.merge(
        pd.read_csv(f, sep="\t", comment="#", index_col=0).iloc[:, 5].to_frame()
    )
...
output_stranded_gtf = snakemake.output.stranded_collapsed_gtf
output_unstranded_gtf = snakemake.output.unstranded_collapsed_gtf

annotation = Annotation(input_gtf)
